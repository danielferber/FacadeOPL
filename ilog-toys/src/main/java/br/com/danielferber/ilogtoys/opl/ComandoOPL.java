 ...
 * <li>Opcionalmente, salvar modelo, dados internos e dados externos em arquivo
 * para depurar com CPLEX Studio.
...
//	public final Logger loggerModelo;
     ...
     */
    protected IloOplModel getOplModel() {
        return oplModel;
    }
    /**
    ...
    /**
     * @return Solver command that guides the execution of the CPLEX or CP
     * instance.
     */
    protected SolverCommand getComandoResolvedor() {
...
//		this.loggerModelo = LoggerFactory.getLogger(this.loggerDados, "modelo");
    ...
    public void executar() throws NoSolutionException {
        ...
        Meter op = MeterFactory.getMeter(loggerMeter, "executeOpl").m("Execute OPL").start();
        try {
            /*
    ...
    /**
     * Registra no log uma cópia dos dados 'externos', que são os dados passados
     * como entrada para o modelo OPL, tipicamente por um arquivo .DAT ou pelos
     * datasources Java. Este arquivo pode ser usado pelo OPL Studio como
     * arquivo .DAT para reproduzir a execução do modelo.
     */
    ...
    /**
     * Registra no log uma cópia dos dados da solução. Este arquivo pode ser
     * comparado com a solução reproduzida do modelo no CPLEX Studio.
     */
    ...
    /**
     * Escreve no arquivo uma cópia dos dados 'internos', que são dados
     * calculados pelo modelo OPL usado dados 'externos' ou dados da solução.
     * Este arquivo pode ser comparado com os dados obtidos com a reprodução do
     * modelo no CPLEX Studio.
     */
    ...
    protected void logSolutionData() {
        if (!loggerSolution.isInfoEnabled()) {
            return;
        }
    ...
    /**
     * Registra no log uma cópia dos dados 'internos', que são dados calculados
     * pelo modelo OPL usado dados 'externos' ou dados da solução.
     */
    protected void logInternalData() {
        if (!loggerExternalData.isInfoEnabled()) {
            return;
        }
    ...
    protected void logExternalData() {
        if (!loggerInternalData.isInfoEnabled()) {
            return;
        }
