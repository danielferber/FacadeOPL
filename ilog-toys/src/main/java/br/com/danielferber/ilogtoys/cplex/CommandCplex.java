...
public class CommandCplex implements SolverCommand {
     ...
     */
    protected final ConfigurationCplex getConfiguration() {
     ...
     */
    public CommandCplex(IloCplex cplexInstance, ConfigurationCplex configuration) {
        super();

        Argument.notNull(cplexInstance);
    ...
    public void execute() throws NoSolutionException {
        ...
        Meter op = MeterFactory.getMeter(loggerMeter, "executeCplex").m("Execute CPLEX").start();
        try {
            /*
            ...
            while (true) {
                iterationCounter++;
                Meter opI = MeterFactory.getMeter(loggerMeter, "iterateCplex").m("Iterate CPLEX").ctx("n", Integer.toString(iterationCounter)).start();
    ...
    protected void executeIteration(int iterationCounter) {
        ...
        try {
            /*
             * TODO Implementar outctxstream para capturar saida do cplex.
    ...
    protected void logSolverProperties() {
        ...
        try {
            ...
            loggerExecucao.warn("Failed to log salver properties.", e);
        }
    }
    ...
    protected void logExecutionProperties() {
        ...
        try {
            ...
            /* Do not interrupt execution. Considered a minor failure. */
            loggerExecucao.warn("Failed to log execution properties.", e);
        }
    }
