 ...
 */
package br.com.danielferber.ilogtoys.opl;

import br.com.danielferber.ilogtoys.cplex.CommandCplex;
import br.com.danielferber.ilogtoys.cplex.ConfigurationCplex;
import br.com.danielferber.ilogtoys.ilog.NoSolutionException;
import br.com.danielferber.ilogtoys.ilog.SolverCommand;
...
/**
 * Executa o OPL conforme as configurações. Isto envolve esconder a maior
 * quantidade possível dos passos necessários segundo a API do OPL para criar o
 * modelo CPLEX.
 *
...
public class FacadeOPL {
    ...
    protected ConfiguracaoOPL getConfiguracaoOpl() {
        return configuracaoOpl;
    }
    ...
    protected ConfigurationCplex getConfiguracaoCplex() {
        return configuracaoCplex;
    }
    /**
     * Acesso ao modelo.
     */
    ...
    public void executar() throws NoSolutionException, OplSettingsException, OplModelException {
        ...
        Meter allOp = MeterFactory.getMeter(loggerMeter, "ExecuteFacade").start();
    ...
    protected SolverCommand createSolver(IloOplFactory oplFactory, CustomErrorHandler errorHandler, IloOplModelDefinition oplModelDefinition) {
        ...
        if (configuracaoCplex != null) {
            Meter op = MeterFactory.getMeter(loggerMeter, "CreateCplex").start();
            try {
    ...
    protected IloOplModel createModelOnSolver(IloOplFactory oplFactory, CustomErrorHandler errorHandler, IloOplModelDefinition oplModelDefinition, CommandCplex comandoSolver) {
        Meter op = MeterFactory.getMeter(loggerMeter, "CreateModelCplex").start();
        try {
    ...
    protected void defineDataSources(IloOplModel oplModel) {
        Meter op = MeterFactory.getMeter(loggerMeter, "DefineDataSources").start();
        try {
    ...
    protected void defineDataSinks(IloOplModel oplModel) {
        Meter op = MeterFactory.getMeter(loggerMeter, "DefineDataSinks").start();
        try {
     ...
     * REGISTRAR
     *
     * Aplicar fontes de dados. A leitura ocorre no futuro na próxima etapa, ou
     * seja, ao gerar o modelo.
     */
    protected void registerDataSources(IloOplModel oplModel) {
        Meter op = MeterFactory.getMeter(loggerMeter, "RegisterDataSources").start();
        try {
            for (DataSource fonte : this.dataSources) {
...
//			throw new MotivoRuntimeException(MotivosExecucao.RegisterDataSources);
    ...
    protected void exportDataSinks(IloOplModel oplModel) {
        Meter op = MeterFactory.getMeter(loggerMeter, "ExportDataSinks").start();
        try {
            for (DataSink consumidor : this.dataSinks) {
...
//			throw new MotivoRuntimeException(MotivosExecucao.ExportDataSinks);
    ...
    protected void realizeModelOnSolver(IloOplFactory oplFactory, CustomErrorHandler errorHandler, IloOplModel oplModel, CommandCplex comandoSolver) {
        Meter op = MeterFactory.getMeter(loggerMeter, "RealizeModelOnCplex").start();
        try {
            oplModel.generate();
            FacadeOPL.throwPossibleOplModelException(errorHandler);
            op.ok();
    ...
    protected void executeSolver(IloOplModel oplModel, SolverCommand comandoSolver) throws NoSolutionException {
        Meter op = MeterFactory.getMeter(loggerMeter, "ExecuteSolver").start();
        try {
    ...
    protected void executePosProcessing(IloOplModel oplModel, CustomErrorHandler errorHandler) {
        Meter op = MeterFactory.getMeter(loggerMeter, "ExecutePosProcessing").start();
        try {
...
//		}
     ...
     * MODEL DEFINITION.
     *
     * Tenta adiantar a validação sintaticamente do modelo. Para isto tenta
     * acessar qualquer uma das definições para forçar a compilação do modelo.
     *
    ...
    protected IloOplModelDefinition parseModel(IloOplFactory oplFactory, CustomErrorHandler errorHandler, IloOplSettings oplSettings, IloOplModelSource oplModelSource) throws OplModelException {
        Meter op = MeterFactory.getMeter(loggerMeter, "ParseModel").start();
        try {
            FacadeOPL.throwPossibleOplModelException(errorHandler);
     ...
     * affirmative.
     *
     * @throws OplModelException if there are {@link OplModelParseError}s
     * reported to the {@link CustomErrorHandler}
     */
     ...
     * <p>
     * This method shall be called from within a
     * <code>catch (Exception e)</code> clause, that wraps all OPL method calls.
     *
     * <pre>
     * try { ... }
     * catch (Exception e) {
    ...
    protected static RuntimeException wrapPossibleSpuriousIloException(Exception e) {
        if (e instanceof IloException) {
            LoggerFactory.getLogger(FacadeOPL.class).warn("JNI has thrown undeclared exception.", e);
        }
    ...
    protected IloOplModelSource loadModel(IloOplFactory oplFactory, CustomErrorHandler errorHandler) throws OplModelException {
        Meter op = MeterFactory.getMeter(loggerMeter, "LoadModel").start();
        try {
...
//			throw new MotivoRuntimeException(e, MotivosExecucao.LoadModel);
     ...
     * CONFIGURAÇÃO.
     *
     * Cria o objeto de configuração que determina o comportamento da execução
     * do OPL. A API do IloOplSettings requer que o error handler seja criado
     * junto com a configuração. Muitas inconsistências são reportadas através
     * de mensagens pelo error handler ao invés de exceções.
     *
    ...
    protected IloOplSettings createSettings(IloOplFactory oplFactory, CustomErrorHandler errorHandler) throws OplSettingsException {
        Meter op = MeterFactory.getMeter(loggerMeter, "CreateSettings").start();
        try {
...
//				throwPossibleOplModelException(errorHandler);
     ...
     * INICIALIZAÇÃO.
     *
     * Acessa a classe da OplFactory e a configura para mostrar erros de
     * inicialização de acordo com a configuração. Isto também força o Java a
     * carregar as DLLs do ILOG, caso isto não tenha ocorrido até então. Se uma
     * DLL não puder ser encontrada, então ocorrerá um
     * ExceptionInInitializerError que será reportado como um MotivoException.
     *
     * A documentação do OPL não deixa claro qual o comportamento esperado em
     * caso de erro de licença. Este caso será tratado de forma genérica, igual
     * ao erro de não encontar DLL. TODO Iniciar só na primeira vez que o Facade
     * for chamado. TODO Imprimir o library path e colocar uma mensagem
     * explicando como definir o library path na linha de comando. TODO Se algo
     * der errado, verificar se a DLL ou SO existe nos paths de interesse. TODO
     * Ser algo der errado, ver cabeçalho da dll/os para ver se ela é 32 ou 64
     * bits. Avisar se for diferente da arquitetura da JVM.
     *
     * @return
     *
    ...
    protected IloOplFactory loadLibrary() throws LoadOplLibraryException {
        Meter op;
        op = MeterFactory.getMeter(loggerMeter, "LoadModel").start();
        try {
